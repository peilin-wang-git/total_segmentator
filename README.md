...
## 5.1 运行时版本基线（回答常见版本问题）

- **Python**：`>=3.10, <3.12`（在 `pyproject.toml` 中显式约束）
- **PyTorch**：`2.2.2`（在 `requirements.txt` 中显式固定）
- **TensorFlow**：**不需要**（本项目不依赖 TensorFlow）

> 说明：TotalSegmentator 当前流程基于 PyTorch 生态，不走 TensorFlow 推理链路。
