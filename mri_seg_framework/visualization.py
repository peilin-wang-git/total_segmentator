...
def save_overlay_slices_jpg(image_path: Path, seg_path: Path, output_dir: Path) -> None:
    img = sitk.GetArrayFromImage(sitk.ReadImage(str(image_path))).astype(np.float32)
    seg = sitk.GetArrayFromImage(sitk.ReadImage(str(seg_path))).astype(np.int16)
    ...
    if img.ndim == 3 and seg.ndim == 3:
        _save_3d_overlay_series(img, seg, output_dir)
        return
    ...
    if img.ndim == 4 and seg.ndim == 4:
        ...
        for t in range(frames):
            ...
            _save_3d_overlay_series(img[t], seg[t], frame_dir)
        return

    # Fallback for shape mismatch or uncommon dims: try first available 3D block.
    if img.ndim >= 3 and seg.ndim >= 3:
        _save_3d_overlay_series(np.asarray(img)[0] if img.ndim > 3 else img, np.asarray(seg)[0] if seg.ndim > 3 else seg, output_dir)


def _save_3d_overlay_series(img_3d: np.ndarray, seg_3d: np.ndarray, output_dir: Path) -> None:
    ...
    for z in range(num_slices):
        ...
        if np.any(seg_slice > 0):
            ax.imshow(np.ma.masked_where(seg_slice == 0, seg_slice), cmap="tab20", alpha=0.45)
        ax.axis("off")
        fig.tight_layout()
